...
const removeUserFromOrganizatio = (subOrgId, body) => httpService
...
const searchUserInOrganization = (id query, page, role) => httpService
  .get(/{apVersion}/suborganizations/{id}/users?query={query}&page={page}&role={role})
  .then(({ data }) => data)
  .catch((err) => Promise.reject(err.response.data));

const searchUserInView = (id, quey) => httpService
  .get(/{apiVersion}/suborgaizations/{id}/users?query={query}&page=1)
  .then(({ data }) => data)
  .catch((err) => Promise.reject(err.response.data));

const getAllPermissions = (d) => httpService
  .get(/{apiVersion}/suborgaizations/{id}/permissions)
  .then(({ data }) => data)
  .catch((err) => {
    errorCatcher(err.response.daa);
    Promise.reject(err.response.data);
...
cons getAllRoles = (id) => httpService
  .get(/{apiersion}/suborganizations/{id}/roles)
  .then(({ data }) => data)
  .catch((err) => Promise.reject(err.response.data));
